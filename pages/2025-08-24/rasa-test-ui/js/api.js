...
> 🚨 **중요**: 3월 31일까지 **필수 설정**해야 합니다!`
  ...
  parseResponse(data) {
    ...
    if (Array.isArray(data?.messages)) {
      // Extract text from each message and join with line breaks
      const textMessages = data.messages
        .filter(msg => msg && msg.text)
        .map(msg => msg.text);
      
      if (textMessages.length > 0) {
        answer = textMessages.join('\n');
      }
    }
    
    // Fallback to other fields if messages not available
    ...
    // If still no answer, use default
    if (!answer) {
